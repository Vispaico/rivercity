...
export default function RiverCityChatBot() {
  ...
  const speak = (text) => {
    if (!('speechSynthesis' in window) || !isVoiceEnabled || !text) return;
  ...
  const handleUserInput = async (text, fromVoice = false) => {
    ...
    try {
      ...
      const botReply = data.content || "Sorry, I didn't understand that.";

      setMessages(prev => [...prev, { role: 'assistant', content: botReply }]);

      // Speak the reply — works for both text and voice input
      if (isVoiceEnabled) {
        speak(botReply);
      }
    } catch (err) {
  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-6 right-36 z-50 w-16 h-16 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-2xl flex items-center justify-center text-3xl transition-all active:scale-95"
      >
        🏍️
      </button>
