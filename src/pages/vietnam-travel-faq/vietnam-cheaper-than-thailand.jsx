      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
        ...
        <div className="container mx-auto px-4 py-12">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="mb-10"><h2 className="text-2xl font-bold text-slate-800 mb-6 flex items-center gap-3"><span className="w-1 h-8 bg-gradient-to-b from-green-500 to-emerald-600 rounded-full"></span>Related Articles</h2><div className="grid md:grid-cols-2 gap-4">{relatedArticles.map((article) => (<Link key={article.id} to={`/vietnam-travel-faq/${article.slug}`} className="group block"><div className="bg-white rounded-xl p-5 shadow-sm hover:shadow-lg transition-all duration-300 border border-slate-200/60 hover:border-green-300/60 h-full"><h3 className="font-semibold text-slate-800 mb-2 group-hover:text-green-600 transition-colors line-clamp-2">{article.question}</h3><p className="text-sm text-slate-600 line-clamp-2 mb-3">{article.shortSnippet}</p><div className="flex items-center text-green-500 text-sm font-medium">Read more<ArrowRight className="w-4 h-4 ml-1 group-hover:translate-x-1 transition-transform" /></div></div></Link>))}</div></div>
            <div className="text-center"><Link to="/vietnam-travel-faq" className="inline-flex items-center gap-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white px-8 py-4 rounded-xl font-semibold hover:shadow-lg hover:shadow-green-500/25 transition-all"><ChevronLeft className="w-5 h-5" />Back to Vietnam Travel FAQ</Link></div>
          </div>
        </div>
      </div>
    </>
